 ...
 * @author George Raptis <georapbox@gmail.com>
 * @license MIT
 */var tm="clipboard-copy",tp="success",tb="error",tg=document.createElement("template"),tf=`
  :host([hidden]),
  [hidden],
  ::slotted([hidden]) {
    display: none !important;
...
`;class tS extends HTMLElement{#I=null;#B=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tL.content.cloneNode(!0))}async connectedCallback(){this.#I=this.shadowRoot?.getElementById("historyList"),this.#B=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#$((await X())[1]||[]),this.#I?.addEventListener("click",this.#D),this.#B?.addEventListener("click",this.#F)}disconnectedCallback(){this.#I?.removeEventListener("click",this.#D),this.#B?.removeEventListener("click",this.#F)}async add(t){let[,e]=await J();if(!t||!e?.addToHistory)return;let[o,i=[]]=await X();if(!o&&Array.isArray(i)&&!i.find(e=>e===t)){let e=[...i,t],[o]=await Q(e);o||this.#I.appendChild(this.#P(t))}}async remove(t){if(!t)return;let[e,o=[]]=await X();if(!e&&Array.isArray(o)){let e=o.filter(e=>e!==t),[i]=await Q(e);if(!i){let e=this.#I.querySelector(`li[data-value="${t}"]`);e&&e.remove()}}}async empty(){let[t]=await Q([]);t||this.#I.replaceChildren()}#$(t){if(!this.#I)return;this.#I.replaceChildren();let e=document.createDocumentFragment();t.forEach(t=>{e.appendChild(this.#P(t))}),this.#I.appendChild(e)}#P(t){let e;let o=window.crypto&&"function"==typeof window.crypto.randomUUID?window.crypto.randomUUID():Array.from({length:5},()=>Math.random().toString(36).substring(2,8)).join("-"),i=document.createElement("li");i.setAttribute("data-value",t);try{new URL(t),(e=document.createElement("a")).href=t,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=t,e.setAttribute("id",`historyItem-${o}`);let r=document.createElement("div");r.className="actions";let s=document.createElement("custom-clipboard-copy");s.setAttribute("id",`copyHistoryItem-${o}`),s.setAttribute("aria-label","Copy to clipboard"),s.setAttribute("aria-labelledby",`copyHistoryItem-${o} historyItem-${o}`),s.setAttribute("only-icon",""),s.setAttribute("value",t),r.appendChild(s);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("id",`removeHistoryItem-${o}`),a.setAttribute("aria-label","Remove from history"),a.setAttribute("aria-labelledby",`removeHistoryItem-${o} historyItem-${o}`),a.innerHTML=`
    ...
    `,r.appendChild(a),i.appendChild(e),i.appendChild(r),i}#D=t=>{let e=t.target;if(e.closest('[data-action="delete"]')){let t=e.closest("li").dataset.value;window.confirm(`Delete history item ${t}?`)&&this.remove(t)}};#F=()=>{window.confirm("Empty history? This action cannot be undone.")&&this.empty()};static defineCustomElement(t="bs-history"){"undefined"==typeof window||window.customElements.get(t)||window.customElements.define(t,tS)}}tS.defineCustomElement(),async function(){let t;let e=document.querySelector("a-tab-group"),o=document.querySelector("video-capture"),i=document.querySelector("bs-settings"),r=document.querySelector("bs-history"),s=document.getElementById("cameraPanel"),a=document.getElementById("filePanel"),n=document.getElementById("scanInstructions"),l=document.getElementById("scanBtn"),d=document.getElementById("dropzone"),c=document.querySelector("resize-observer"),h=document.getElementById("scanFrame"),u=document.getElementById("torchButton"),m=document.getElementById("globalActions"),p=document.getElementById("historyBtn"),b=document.getElementById("historyDialog"),g=document.getElementById("settingsBtn"),f=document.getElementById("settingsDialog"),v=document.getElementById("settingsForm"),y=document.getElementById("cameraSelect"),w=!0;"function"==typeof HTMLDialogElement&&(m?.removeAttribute("hidden"),b?.removeAttribute("hidden"),f?.removeAttribute("hidden"));let{barcodeReaderError:E}=await tn.setup();if(E){let t=document.getElementById("barcodeReaderError");w=!1,m?.setAttribute("hidden",""),e?.setAttribute("hidden",""),t?.removeAttribute("hidden"),t.textContent=E?.message;return}let A=await tn.getSupportedFormats(),[,x]=await J(),k=x?.formats||A,C=await tn.create(k);o.addEventListener("video-capture:video-play",_,{once:!0}),o.addEventListener("video-capture:error",function(t){let e=t.detail.error;if("NotFoundError"===e.name)return;let o="NotAllowedError"===e.name?"Permission to use webcam was denied or video Autoplay is disabled. Reload the page to give appropriate permissions to webcam.":e.message;s.innerHTML=`<div class="alert alert-danger" role="alert" style="margin: 0;">${o}</div>`},{once:!0}),tu.defineCustomElement();let L=o?.shadowRoot,S=L?.querySelector("video"),z=L?.querySelector('[part="actions-container"]');async function T(){n?.removeAttribute("hidden");try{let e=await C.detect(S),o=e?.rawValue??"";if(!o)throw Error(F);window.cancelAnimationFrame(t),te(s,o),r?.add(o),n?.setAttribute("hidden",""),l?.removeAttribute("hidden"),h?.setAttribute("hidden",""),z?.setAttribute("hidden",""),tr();return}catch{}w&&(t=window.requestAnimationFrame(()=>T()))}async function _(t){h?.removeAttribute("hidden"),ts(t.detail.video,h),T();let e=t.target.getTrackSettings(),i=t.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(u?.addEventListener("click",M),u?.removeAttribute("hidden"),o.hasAttribute("torch")&&tl({el:u,isTorchOn:!0})),e?.zoom&&i?.zoom){let t=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=e?.zoom||1;t?.addEventListener("click",t=>{let e=t.target.closest('[data-action="zoom-in"]'),i=t.target.closest('[data-action="zoom-out"]');e&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n.toFixed(1),o.zoom=n}),t?.removeAttribute("hidden"),r.textContent=n.toFixed(1)}let s=await tu.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,y.appendChild(o)}),s.length>1&&(y?.addEventListener("change",q),y?.removeAttribute("hidden"))}async function R(t){t.preventDefault();let e={},o=new FormData(v),i=o.getAll("general-settings"),r=o.getAll("formats-settings");i.forEach(t=>e[t]=!0),e.formats=r,K(e),"formats-settings"===t.target.name&&(C=await tn.create(r))}function M(t){o.torch=!o.torch,tl({el:t.currentTarget,isTorchOn:o.hasAttribute("torch")})}function q(t){if("function"!=typeof o.restartVideoStream)return;let e=t.target.value||void 0;o.restartVideoStream(e)}d.accept=P.join(","),i.supportedFormats=A,l.addEventListener("click",function(){l?.setAttribute("hidden",""),h?.removeAttribute("hidden"),z?.removeAttribute("hidden"),s&&s.querySelector("bs-result")?.remove(),T()}),e.addEventListener("a-tab-show",tt(function(t){let e=t.detail.tabId,o=document.querySelector("video-capture");if("cameraTab"===e){if(w=!0,!o)return;let t=null!=s.querySelector("bs-result");if(o.loading||t||(h?.removeAttribute("hidden"),z?.removeAttribute("hidden"),T()),"function"==typeof o.startVideoStream){let t=y?.value||void 0;o.startVideoStream(t)}}else"fileTab"===e&&(w=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream(),h?.setAttribute("hidden",""),z?.setAttribute("hidden",""))},250)),d.addEventListener("files-dropzone-drop",function(t){!function(t){if(!t)return;let e=new Image,o=new FileReader;o.onload=o=>{let i=o.target.result;e.onload=async()=>{try{let t=await C.detect(e),o=t?.rawValue??"";if(!o)throw Error(F);te(a,o),r?.add(o),tr()}catch(t){te(a,F),tr({success:!1})}},e.src=i,e.alt="Image preview",d.replaceChildren();let s=document.createElement("div");s.className="dropzone-preview";let n=document.createElement("div");n.className="dropzone-preview__image-wrapper";let l=document.createElement("div");l.className="dropzone-preview__file-name",l.textContent=t.name,n.appendChild(e),s.appendChild(n),s.appendChild(l),d.prepend(s)},o.readAsDataURL(t)}(t.detail.acceptedFiles[0])}),c.addEventListener("resize-observer:resize",function(){ts(o.shadowRoot.querySelector("video"),h)}),g.addEventListener("click",function(){f.open=!0}),v.addEventListener("change",tt(R,500)),p.addEventListener("click",function(){b.open=!0}),document.addEventListener("visibilitychange",function(){if("cameraTab"===e.querySelector("[selected]").getAttribute("id")){if("hidden"===document.visibilityState)w=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream();else{w=!0;let t=document.querySelector("video-capture");if(!t)return;if(t.loading||s.querySelector("bs-result")||T(),"function"==typeof t.startVideoStream){let e=y?.value||void 0;t.startVideoStream(e)}}}}),document.addEventListener("keydown",function(t){"Escape"===t.key&&function(){let t=e.querySelector("#cameraTab").hasAttribute("selected"),o=!l.hidden,i=f.hasAttribute("open"),r=b.hasAttribute("open");o&&t&&!(i||r)&&l.click()}()})}()}();
//# sourceMappingURL=index.03482571.js.map
